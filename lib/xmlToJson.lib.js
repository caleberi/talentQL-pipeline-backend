...
function xml2jsonRecurse(xmlStr) {
  ...
  while (xmlStr.match(/<[^\/][^>]*>/)) {
    ...
    if (inner_substring.match(/<[^\/][^>]*>/)) {
      tempVal = xmlToJson(inner_substring);
    } else {
    ...
    xmlStr = xmlStr.substring(
      openingTag.length * 2 + 1 + inner_substring.length
    );
  }
...
function replaceAloneValues(xmlStr) {
  var tagsWithAttributesAndValue = xmlStr.match(
    /<[^\/][^>][^<]+\s+.[^<]+[=][^<]+>{1}([^<]+)/g
  );
